...
I really like the combination of the time trend with the number of cases, but because of the spurious correlation, I want to have it represent something else. Would it be possible to simply plot the number of cases and deaths for the entire first year (maybe until April 23 2021) and then on top of that show the number of entries per day for the days during which the diaries were written. The purpose is simply to show that these thoughts reflect a time when cases were actually very low to what came later. In that sense, they should be seen as reflections within a very specific time, when the future was very unclear.

```{r}
df_covid <- read.csv("data/owid-covid-data.csv")

g1 <- df_covid %>% as_tibble() %>%
...
g2 <- df %>% 
  ...
  labs(y = "Number of diary entries",
       x = NULL)
