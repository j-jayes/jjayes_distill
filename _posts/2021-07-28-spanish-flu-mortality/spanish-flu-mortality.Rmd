...
### Correlates and causes of pandemic mortality

A wealth of research has sought to examine what features explain the differing levels of mortality in different communities during the pandemic. @AcuaSoto2011InfluenzaAP examine features correlated with mortality from the flu in 66 large cities across the United States. The authors find that baseline health status was important, as proxied by pre-pandemic pneumonia, which they argue is "a bacterial condition with a strong biological interaction with the influenza virus" and infant mortality rates as a surrogate for population health [@AcuaSoto2011InfluenzaAP, p. 13]. @Clay2018 study the effect of pollution from coal fired power plants on pandemic mortality in the United States, and find that poor air quality was "an important cause of mortality during the pandemic" [@Clay2018, p. 79].

### Calculating excess mortality

@AcuaSoto2011InfluenzaAP present a clear method for assessing excess mortality based on a comparison with the historical trend. They define excess mortality as "deaths observed during the pandemic years minus the average death counts in the pre-pandemic years" [@AcuaSoto2011InfluenzaAP, p.23]. The authors had access to seven years of data preceding the pandemic. In addition, they normalize the death counts by population for the inhabitants of their 66 cities in the sample.

### Who died

The question of who died during the pandemic in South Africa has recieved some recent investigation. @Fourie

## Data

The sources of data available to a researcher without doubt shape the design of their study. In his authoritative account of the Spanish flu pandemic in South Africa, historian Howard Phillips notes that prior to his own study, “extensive documentation... from [unpublished evidence] in the report of the Government-appointed Commission of Inquiry" had laid on the shelves of the Library of Parliament unexamined [@Phillips1984, p.9]. Similarly, the digitization of death records allows me at this time to ask different kinds of questions about the pandemic of 1918. Speficially it opens up an assessment of excess mortality and an indicatation of the importance of pre-existing levels of health in determining pandemic mortality rates within the districts of the Cape.
...
Race is one important feature of the analysis. As noted in @Fourie2021, health and wealth in the Cape Province of 1918 were "closely correlated to race... with race being a strong predictor of income and social status" [@Fourie2021, p. 2]. The death notices provide a field for the race of the deceased to be recorded. However, often ethnicity of the deceased was recorded instead. See for example the record of Wills Bunu, who died in Queenstown on the 26th of October 1918 whose race is recorded as "Fingo". Fingo is a term for a group of ethnicities who fled Natal in the aftermath of the *Mfecane* to reside in the Xhosa speaking Eastern region of the Cape [@Stapleton, p. 362]. @Mtumane argues that the Fingo were "were embraced and integrated" by Xhosa in the Eastern Cape by the time of the Spanish flu [@Mtumane, p. 68]. 


![Form of Information of the death of Wills Bunu](images/wills_bunu.jpg)


Further, more than 200 ethnicities appear only once among the records, such that the distribution of observed ethnicities is skewed to the right, with 419 *different* ethnicities recorded.
...
df %>% 
  ...
  scale_x_log10() +
  labs(x = "Number of observations of a recorded ethnicity",
...
An alternate set of methods for quantifying excess mortality use history as a guide for the number of deaths that we can expect to occur in each district. Three methods use the mortality figures for three years preceding the pandemic as a guide, first looking at the average number of deaths in each district during the pandemic peak (October, November and December), second taking an average of the whole year mortality in each district, and third creating regression models with the historical data to predict the expected number of deaths in the pandemic period. I explain each below.

First, making use of the death notices from 1915 to 1917 and following the methodology laid out by the South Africa Medical Research Council in @Dorrington2021, I calculate the average number of deaths per month in each town during the pandemic period in the three preceding years as a baseline. The SAMRC define excess mortality is calculated as "the number of all-cause deaths in the last week less the number that might have been expected to have occurred" [@Dorrington2021, p. 2]. I widen the bandwidth from one week to one month to the coarsity of the data at district level. The resulting smoother estimates have less noise and appear reasonable. I then subtract this expected number of deaths from the observed number during the pandemic to calculate excess mortality during the 3 months of pandemic peak in 1918. The advantage of this method is that it captures the seasonal variation in mortality, as shown in Figure \@ref(fig:seasonality). It is interesting to note that the the smoothed line shows a peak in mortality in the third qarter of the year in the Cape between 1915 and 1920. This is in contrast to the seasonal pattern in mortality in South Africa as a whole in since 2015 as shown by @Dorrington2021 which displays a dramatic peak during winter.
...
This first method results in estimates of excess mortality of 
...
Second, following the methodology of @AcuaSoto2011InfluenzaAP I calculate a simple average of the number of deaths per year in each district in the three prior years and subtract the number of deaths in 1918. Table \@ref(tab:mean-mortality-by-district-tab) shows the mortality by district from 1915 to 1917, and the average across these three years.
...
Figure \@ref(fig:excess-mortality-method-2) shows the excess mortality based on method two. 

```{r excess-mortality-method-2, layout="l-body-outset", fig.width = 8, fig.height=6, fig.cap="Mortality by week across districts - excluding 1918"}
df %>%
  mutate(year = year(death_date)) %>%
  count(year, town) %>%
  filter(year == 1918) %>%
...
```{r, pneumonia-influenza-mort, layout="l-body-outset", fig.width = 8, fig.height=6, fig.cap="Correlation pneumonia and influenza mortality"}
...
"Whites born in South Africa seemed to be more vulnerable [to the flu] than those who had grown up in Europe." Phillips quotes Dr W Watkins-Pitchford, Director of South African Institute for Medical Research, who said that "It was probable that most of the [European born whites] had acquired a certain amount of immunity in previous epidemics in Europe" [@Phillips1984, p. 295]
...
By tuning the number of tokens using the 10 folds of cross validation data I can seect the 
...
Finally, with the hyper paramter value chosen, I fit the model a last time, training on the training data and tested on the testing data created with the initial split. 
