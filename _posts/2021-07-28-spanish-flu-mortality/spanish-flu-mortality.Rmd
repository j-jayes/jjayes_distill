...
The output of the classification process is shown in \@ref(fig:ethnicity-race-classification) for the 12 most common classes for each race.
...
<tr><td colspan="3" style="border-bottom: 1px solid black"></td></tr><tr><td style="text-align:left">child_mort_rate</td><td>0.095</td><td>0.066<sup>*</sup></td></tr>
...
First I calculate the pre-pandemic mortality rates by district from influenza, pneumonia and both. These are shown in Table [cite] below.
...
pneumonia_influenza %>% 
  group_by(town) %>% 
  mutate(across(c(influenza_deaths, pneumonia_deaths, total), ~ sum(.x, na.rm = T))) %>% 
  ungroup() %>% 
  distinct(town, influenza_deaths, pneumonia_deaths, total) %>% 
  mutate(influenza_mort = influenza_deaths / total,
         pneumonia_mort = pneumonia_deaths / total,
         pneumonia_influenza_mort = (influenza_deaths + pneumonia_deaths) / total) %>% 
  inner_join(pneumonia_influenza_mort %>% 
  select(town, deaths) %>% 
  group_by(town) %>% 
  mutate(deaths = sum(deaths)) %>% 
  ungroup() %>% 
  distinct(town, deaths) %>% 
  filter(!is.na(town))) %>% 
  mutate(pandemic_mort = deaths / total) %>% 
  mutate(race = "Both groups") %>% 
  bind_rows(pneumonia_influenza) %>% view()
...
<tr><td style="text-align:left"></td><td colspan="2">mort_rate</td></tr>
<tr><td style="text-align:left"></td><td>(1)</td><td>(2)</td></tr>
<tr><td colspan="3" style="border-bottom: 1px solid black"></td></tr><tr><td style="text-align:left">share_literate</td><td>-0.009</td><td>0.019</td></tr>
<tr><td style="text-align:left"></td><td>(0.018)</td><td>(0.014)</td></tr>
<tr><td style="text-align:left"></td><td></td><td></td></tr>
<tr><td style="text-align:left">raceWhite</td><td></td><td>-0.042<sup>***</sup></td></tr>
<tr><td style="text-align:left"></td><td></td><td>(0.011)</td></tr>
<tr><td style="text-align:left"></td><td></td><td></td></tr>
<tr><td style="text-align:left">Constant</td><td>0.037<sup>***</sup></td><td>0.036<sup>***</sup></td></tr>
<tr><td style="text-align:left"></td><td>(0.009)</td><td>(0.006)</td></tr>
<tr><td style="text-align:left"></td><td></td><td></td></tr>
<tr><td colspan="3" style="border-bottom: 1px solid black"></td></tr><tr><td style="text-align:left">Observations</td><td>14</td><td>28</td></tr>
<tr><td style="text-align:left">R<sup>2</sup></td><td>0.020</td><td>0.523</td></tr>
<tr><td style="text-align:left">Adjusted R<sup>2</sup></td><td>-0.062</td><td>0.485</td></tr>
<tr><td style="text-align:left">Residual Std. Error</td><td>0.017 (df = 12)</td><td>0.016 (df = 25)</td></tr>
<tr><td style="text-align:left">F Statistic</td><td>0.245 (df = 1; 12)</td><td>13.694<sup>***</sup> (df = 2; 25)</td></tr>
<tr><td colspan="3" style="border-bottom: 1px solid black"></td></tr><tr><td style="text-align:left"><em>Note:</em></td><td colspan="2" style="text-align:right"><sup>*</sup>p<0.1; <sup>**</sup>p<0.05; <sup>***</sup>p<0.01</td></tr>
</table>
